...
# ---------------------------------------------------------------------------
# Skip guard: e2e server tests require the full FastAPI stack including
# Vertex AI / GCP Application Default Credentials for telemetry.
# These tests are designed to run in cloud CI/CD environments.
# ---------------------------------------------------------------------------
_use_vertex = os.getenv("GOOGLE_GENAI_USE_VERTEXAI", "false").lower() in ("true", "1")
pytestmark = pytest.mark.skipif(
    not _use_vertex,
    reason=(
        "GOOGLE_GENAI_USE_VERTEXAI is not set to 'true' — "
        "e2e server tests require GCP Application Default Credentials "
        "and Vertex AI. These tests are designed for cloud CI/CD environments."
    ),
)
